            ...
            try:
                ...
                if 'payload' in data and len(data['payload']) > 0:
                    ...
                    if quat is not None and acc is not None:
                        ...
                        # Original expects 6 IMUs, we only have 1 phone, so replicate to position 3 (right pocket)
                        # and leave others as zeros
                        quat_full = np.zeros((6, 4))
                        acc_full = np.zeros((6, 3))
                        
                        # Place phone data at position 3 (right pocket) - matching 'rp' combo
                        quat_full[3] = quat
                        acc_full[3] = acc * -9.8  # Scale acceleration like original
...
if __name__ == '__main__':
    ...
    while running:
        ...
        ori_raw, acc_raw = imu_set.get_current_buffer() # [buffer_len, 6, 4] and [buffer_len, 6, 3]
        
        if ori_raw.shape[0] == 0:
            time.sleep(0.01)
            continue
        
        # Take only first 5 IMUs (exclude pelvis at index 5)
        ori_raw = ori_raw[:, :5, :]  # [buffer_len, 5, 4]
        acc_raw = acc_raw[:, :5, :]  # [buffer_len, 5, 3]
        
        ori_raw = quaternion_to_rotation_matrix(ori_raw).view(-1, n_imus, 3, 3)
        glb_acc = (smpl2imu.matmul(acc_raw.view(-1, n_imus, 3, 1)) - acc_offsets[:5]).view(-1, n_imus, 3)
        glb_ori = smpl2imu.matmul(ori_raw).matmul(device2bone[:5])
    ...
    if args.save:
        ...
        torch.save(data, paths.dev_data / f'phone_dev_{int(time.time())}.pt')
